<?php
// Page that verifies an account's email address with a 6-digit code during connection
session_start();
...
if (isset($_SESSION['verify_user_id_expires_at']) && $_SESSION['verify_user_id_expires_at'] < time()) {
...
if ($_SERVER["REQUEST_METHOD"] === "POST" && isset($_POST["token"])) {
    ...
    if ($token === "") {
        header("Location: verify_email_connection.php?error=empty");
        exit();
    }
    try {
        $stmt = $cnx->prepare("SELECT * FROM auth_token WHERE user_id = :user_id AND token = :token AND type = 'verify_email' AND expires_at >= NOW()");
        ...
        if ($auth) {
            ...
            $update = $cnx->prepare("UPDATE user SET is_verified = 1 WHERE id = :id");
            ...
            header("Location: verify_email_connection.php?error=invalid");
        }
        exit();
    } catch (PDOException $e) {
        header("Location: verify_email_connection.php?error=db");
        exit();
    }
}

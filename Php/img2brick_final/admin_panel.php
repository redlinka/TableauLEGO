    ...
    <div class="tab-content" id="adminTabsContent">
        ...
        <div class="tab-pane fade" id="orders" role="tabpanel">
            <div class="table-container">
                ...
                if (!function_exists('getOriginalImage')) {
                    function getOriginalImage($cnx, $image_id) {
                        $stmt = $cnx->prepare("SELECT filename FROM IMAGE WHERE image_id = ?");
                        $stmt->execute([$image_id]);
                        return $stmt->fetch(PDO::FETCH_ASSOC) ?: ['filename' => 'unknown'];
                    }
                }
                if (!function_exists('getTilingStats')) {
                    function getTilingStats($pavage_txt) {
                        $path = __DIR__ . "/users/tilings/" . $pavage_txt;
                        if (!file_exists($path)) return ['price' => 0, 'quality' => 0];
                        $content = file_get_contents($path);
                        $lines = explode("\n", $content);
                        $quality = 0;
                        $price = 0;
                        foreach ($lines as $line) {
                            if (str_contains($line, "QUALITY:")) $quality = (int)filter_var($line, FILTER_SANITIZE_NUMBER_INT);
                            if (str_contains($line, "PRICE:")) $price = (int)filter_var($line, FILTER_SANITIZE_NUMBER_INT);
                        }

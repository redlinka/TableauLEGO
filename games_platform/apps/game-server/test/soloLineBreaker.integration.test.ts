...
describe("solo line_breaker integration", () => {
  ...
  beforeAll(async () => {
    ...
    const config: AppConfig = {
      ...
      mongodbDbName: "tableaulego_games_line_breaker_test",
      gameTokenSecret: "phase-c-test-secret",
      phpGameTokenSecret: "",
  ...
  it("creates, plays and finalizes a solo line_breaker session", async () => {
    ...
    while (!currentState.finished && currentState.currentOffer && safetyCounter < 16) {
    ...
    expect(currentState.lineClearStats.totalLinesCleared).toBeGreaterThanOrEqual(0);

    const resultResponse = await app.inject({
      method: "GET",
      url: `/api/v1/solo/line-breaker/sessions/${currentState.session.sessionId}/result`,
      headers: {
        authorization: `Bearer ${accessToken}`
      }
    });
    const result = resultResponse.json() as SoloLineBreakerResultResponse;

    expect(resultResponse.statusCode).toBe(200);
    expect(result.finished).toBe(true);
    expect(result.lineClearStats.totalLinesCleared).toBe(
      currentState.lineClearStats.totalLinesCleared
    );
